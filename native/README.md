...
- 主界面沿用旧版 Python GUI 的布局：顶部输入/输出路径、左侧分组参数、右侧大图预览、底部操作按钮。
...
- 合成预览以绿色板面、金色沉金图案和白色丝印显示；导出时分别写入铜箔/阻焊开窗层和顶层丝印层。
...
1. 选择或添加图片后会创建图层；修改参数只更新当前选中的图层。
2. 点击候选的“应用”按钮。
3. 在右侧设置每层的沉金/丝印类型、可见性和叠放顺序；点击“保存 PNG”可保存当前图层。
4. 点击“生成PCB文件”，程序会合并所有可见且已处理的图层，按原图 DPI 计算图案尺寸，并在输出目录生成同名 `.epro2`。状态栏会显示图案和板框的毫米尺寸。
